...
const create_element =
  ...
  (props = {}) => {
    ...
    if (props.class_list)
      props.class_list.forEach((c) => element.classList.add(c));
...
const create_module_selector = () => {
  return ce_div({
    id: "module-selector",
    children_list: ["Spr-A", "Spr-B", "Spr-C", "Aut-A", "Aut-B", "Aut-C"].map(
      (module) =>
        ce_label({
          children_list: [
            ce_input({
          ...
          innerText:
            lang == "ja"
              ? module.replace("Spr-", "春").replace("Aut-", "秋")
              : module,
        })
    ),
  });
...
const create_year_selector = (arr) => {
  return ce_select({
    id: "year-selector",
    children_list: [...new Set(arr.map((c) => c.year))]
      .sort((a, b) => b - a)
      .map((year) =>
        ce_option({
          innerText: year,
...
const create_option_bar = (arr) => {
  return ce_div({
    ...
    children_list: [create_year_selector(arr), create_module_selector()],
  });
};
...
const create_timetable = (arr) => {
  return ce_div({
    ...
    children_list: [
      ...
      ce_div({
        id: "m-timetable-sub",
        class_list: [lang == "en" && "m-timetable-sub-en"],
      }),
    ],
  });
};
...
const create_course = (placable) => (course) => {
  return ce_a({
    children_list: [
      ce_div({
        children_list: placable
          ? [create_course_title(course), create_course_notice(course)]
          : [
...
const get_selected_course_arr = (selected_year, selected_module, arr) => {
  const selected_year_course_arr = arr.filter(
    (course) => course.year == selected_year
  );
  return {
    placable_arr: selected_year_course_arr.filter(
      (course) => course.schedule[selected_module] != null
    ),
    un_placable_arr: selected_year_course_arr.filter(
      (course) => typeof course.schedule == "string"
    ),
  };
};
...
const set_placable_courses = (course_arr) => {
  ...
  course_arr
    .map((course) =>
      course.schedule[selected.module].map((sch) => {
        count -= sch.time.length;
        const c = create_placable_course(course);
        const column_start =
          ["月", "火", "水", "木", "金"].indexOf(sch.day) + 1;
...
const update_courses = (course_arr) => {
  const selected_course_arr = get_selected_course_arr(
    selected.year,
    selected.module,
    course_arr
  );
...
window.addEventListener("load", () => {
  ...
  const course_arr = [
    ...
    return format == "list"
      ? {
          ...
          ["notice"]:
            course.getElementsByClassName("course-card-status")[0].children,
        }
      : {
          // format : card
          ...
          ["year"]: parseInt(
            course.getElementsByClassName("courseitemdetail-date")[0].innerText
          ),
          ["schedule"]: format_schedule(
            course.querySelector("dd.courseitemdetail > span").innerText
          ),
          ["notice"]:
            course.getElementsByClassName("course-card-status")[0].children,
        };
  ...
  if (
    ...
    document.querySelector(
      `#year-selector > option[value="${selected.year}"]`
    ) == null
  ) {
  ...
  document.querySelector(
    `#module-selector input[value="${selected.module}"]`
  ).checked = true;
  ...
  document
    .querySelectorAll(`input[type="radio"][name="module"]`)
